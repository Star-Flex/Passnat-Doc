...
# Minecraft:Java Server 中的应用(Windows)
...
2. **下载并安装 Java**
 ...
 - [本站下载](http://doc.passnat.com/download)
 - [Microsoft 维护的 OpenJDK 版本](https://learn.microsoft.com/zh-cn/java/openjdk/download)
 - [Oracle](https://www.oracle.com/cn/java/technologies/downloads/#jdk17-windows)

 :::tip

    - 提示: 请注意下载来源，可以从本站下载也可以从以上渠道下载，其余渠道下载请注意甄别是否安全。

   :::

 :::info
   - Minecraft 不同版本使用的 Java 版本如下：
   ...
   - 建议使用与服务器版本匹配的 Java 版本，以确保最佳性能和兼容性。
   :::

 - 双击 MSI 或 EXE 文件(如果你下载的文件是这样的话)，将会启动 Windows 应用程序安装程序，你可以根据屏幕上的说明完成安装。
 :::tip

   - 小技巧1： 如果你下载的文件是 ZIP 包，请将其解压缩到你选择的文件夹中，然后将 JAVA_HOME 环境变量设置为该文件夹。
   - 小技巧2：小技巧1实在不会可以在下载 JDK 的时候选择 MSI 或 EXE 文件。
...
3. **首次启动服务器**：
 ...
 - 使用以下命令启动服务器：
  ...
  java -Xmx6G -Xms1024M -jar 你下载服务端文件名字.jar nogui
   ```
  :::info
    - `-Xmx6G` 表示最大内存使用 6GB ，你可以根据需要调整这个值。
    - `-Xms1024M` 表示初始内存使用 1024MB 。
    - `nogui` 表示不使用 gui 。
   :::
...
4. **接受 EULA**：
 ...
 - 打开 `eula.txt` 文件，将 `eula=false` 改为 `eula=true`， __保存__ 并关闭文件。
...
2. **连接服务器**：
 - 启动 Minecraft 客户端，选择多人游戏，点击添加服务器，输入使用 PassNAT 穿透得到的 IP 地址和端口，格式为 `ip:端口` ，即可连接到你的服务器。
